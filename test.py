...
class FinancialAnalyticsPlatform:
    ...
    def _render_enhanced_penman_nissim_mapping(self, data: pd.DataFrame) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            # FIX: Initialize current_mappings early
            current_mappings = st.session_state.get('temp_pn_mappings', {})
            
            # FIX: Initialize validation and unmapped variables
            ...
            # Update current_mappings after potential template change
            current_mappings = st.session_state.get('temp_pn_mappings', {})
            validation = pn_mapper.validate_mappings(current_mappings)
            unmapped = [item for item in source_metrics if item not in current_mappings]
            ...
            if st.session_state.get('show_save_dialog', False):
                with st.form("save_template_form"):
                    ...
                    template_name = st.text_input(
                        "Template Name",
                        value=f"{company_name}_{datetime.now().strftime('%Y%m%d')}",
                    ...
                    with col1:
                        if st.form_submit_button("💾 Save", type="primary"):
                            if template_name and 'temp_pn_mappings' in st.session_state and len(current_mappings) > 0:
                                try:
                                ...
                                st.error("Template name is required and mappings must not be empty")
            ...
            with tabs[2]:
                ...
                for target in pn_mapper.required_mappings['optional']:
                    ...
                    candidates = candidates[:10]  # Limit candidates for optional mappings
            ...
            if st.button("🔧 Recover Lost Cash Flow Items", key="recover_cf_items", type="secondary"):
                with st.spinner("Searching for cash flow items..."):
                    original_files = self.get_state('uploaded_files', []) if hasattr(self, 'get_state') else []
                    ...
                    if original_files:
                        ...
                        for file in original_files:
                            try:
                                if hasattr(self, '_parse_single_file'):
                                    df = self._parse_single_file(file)
                                    if df is not None:
                                        for idx in df.index:
                                            idx_lower = str(idx).lower()
                                ...
                                if hasattr(self, 'logger'):
                                    ...
                                    st.error(f"Error recovering from {file.name}: {e}")
                                continue
                        ...
                        if recovered_items:
                            ...
                            if st.button("Re-process with Cash Flow Preservation", key="reprocess_with_cf"):
                                ...
                                if hasattr(self, '_process_uploaded_files'):
            ...
            if st.checkbox("🔧 Show CapEx Detection Details", key="show_capex_details"):
                with st.expander("Capital Expenditure Detection Analysis", expanded=True):
                    try:
                        ...
                        st.error(f"Error in CapEx detection: {e}")
                ...
                if st.button("🧪 Run Full Data Preservation Test", key="preservation_test"):
                    with st.spinner("Running comprehensive data preservation test..."):
                        ...
                        if capex_items:
                            ...
                            try:
                                temp_analyzer = EnhancedPenmanNissimAnalyzer(data, current_mappings_debug or {})
                                clean_data = temp_analyzer._df_clean
            ...
            st.error(f"An error occurred in the mapping interface: {e}")
            if hasattr(self, 'logger'):
                self.logger.error(f"Error in _render_enhanced_penman_nissim_mapping: {e}")
            st.exception(e)
            return None
