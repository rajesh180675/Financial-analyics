...
class EnhancedPenmanNissimAnalyzer:
    ...
    def _reformulate_income_statement_enhanced(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Version 9.0 - Complete rewrite with correct EBIT calculation and prior year adjustments
        """
        # Use cached result if available
        ...
        self.logger.info("\n" + "="*80)
        self.logger.info("[PN-IS-V9.0] Starting Enhanced Income Statement Reformulation")
        self.logger.info("="*80)
        ...
        try:
            # ========== 1. CORE REVENUE ==========
            # Get net revenue (after excise duty)
            revenue_net = self._get_safe_series(df, 'Revenue From Operations(Net)')
            if revenue_net is None:
                revenue_net = self._get_safe_series(df, 'Revenue')
            
            if revenue_net is None:
                # Try to calculate from gross revenue
                revenue_gross = self._get_safe_series(df, 'Revenue From Operations')
                excise_duty = self._get_safe_series(df, 'Excise Duty', default_zero=True)
                if revenue_gross is not None:
                    revenue_net = revenue_gross - excise_duty
                    self.logger.info(f"[PN-IS-V9.0] Calculated net revenue from gross - excise")
                else:
                    raise ValueError("Cannot find revenue data")
            
            reformulated['Revenue'] = revenue_net
            self.logger.info(f"[PN-IS-V9.0] Revenue range: {revenue_net.min():,.0f} to {revenue_net.max():,.0f}")
            
            # ========== 2. GET KEY COMPONENTS ==========
            # Profit Before Exceptional Items and Tax (this includes the effect of finance cost)
            pbit_before_exceptional = self._get_safe_series(df, 'Profit Before Exceptional Items and Tax')
            if pbit_before_exceptional is None:
                raise ValueError("Cannot find Profit Before Exceptional Items and Tax")
            
            # Finance Cost (Interest Expense)
            finance_cost = self._get_safe_series(df, 'Finance Cost', default_zero=True)
            if (finance_cost == 0).all():
                finance_cost = self._get_safe_series(df, 'Interest Expense', default_zero=True)
            
            # Other Income
            other_income = self._get_safe_series(df, 'Other Income', default_zero=True)
            
            # Prior Year Adjustments
            prior_year_adj = self._get_safe_series(df, 'Prior Year Adjustments', default_zero=True)
            
            # Exceptional Items
            exceptional_items = self._get_safe_series(df, 'Exceptional Items Before Tax', default_zero=True)
            
            self.logger.info(f"[PN-IS-V9.0] Key components:")
            self.logger.info(f"  PBIT before exceptional: {pbit_before_exceptional.to_dict()}")
            self.logger.info(f"  Finance Cost: {finance_cost.to_dict()}")
            self.logger.info(f"  Other Income: {other_income.to_dict()}")
            self.logger.info(f"  Prior Year Adjustments: {prior_year_adj.to_dict()}")
            
            # ========== 3. CALCULATE EBIT AND OPERATING INCOME ==========
            # CRITICAL: In Indian accounting, Finance Cost is already deducted from operating profit
            # So we need to add it back to get EBIT
            ebit_with_other_income = pbit_before_exceptional + finance_cost
            
            self.logger.info(f"[PN-IS-V9.0] EBIT (with Other Income) = PBIT + Finance Cost")
            self.logger.info(f"[PN-IS-V9.0] EBIT = {ebit_with_other_income.to_dict()}")
            
            # Determine if Other Income is operating or non-operating
            # For Indian companies, Other Income typically includes:
            # - Interest income (non-operating)
            # - Dividend income (non-operating)
            # - Profit on sale of investments (non-operating)
            # - Foreign exchange gains (can be operating or non-operating)
            # - Miscellaneous income (mixed)
            
            # Default assumption: Other Income is non-operating (conservative for financial analysis)
            other_income_is_operating = False
            
            # Check if Other Income is significant and likely operating
            if revenue_net is not None and other_income is not None:
                other_income_pct = (other_income / revenue_net * 100).mean()
                
                # If Other Income is > 3% of revenue, investigate further
                if other_income_pct > 3:
                    # Check for patterns suggesting operating income
                    other_income_volatility = other_income.std() / other_income.mean() if other_income.mean() != 0 else 0
                    
                    # High volatility suggests non-operating (investment gains/losses)
                    # Low volatility suggests operating (regular business income)
                    if other_income_volatility < 0.5:
                        other_income_is_operating = True
                        self.logger.info(f"[PN-IS-V9.0] Other Income appears stable ({other_income_volatility:.2f} volatility) - treating as operating")
            
            # Calculate pure operating income
            if other_income_is_operating:
                pure_operating_income = ebit_with_other_income
                total_non_operating_income = pd.Series(0, index=df.columns)
                metadata['other_income_treatment'] = 'Treated as operating income'
            else:
                pure_operating_income = ebit_with_other_income - other_income
                total_non_operating_income = other_income
                metadata['other_income_treatment'] = 'Treated as non-operating income'
            
            # Adjust for prior year items (these affect operating income)
            operating_income_before_tax = pure_operating_income + prior_year_adj
            
            reformulated['Operating Income Before Tax'] = operating_income_before_tax
            self.logger.info(f"[PN-IS-V9.0] Operating Income Before Tax (after prior year adj): {operating_income_before_tax.to_dict()}")
            
            # ========== 4. FINANCIAL ITEMS ==========
            # Net financial income/expense before tax
            net_financial_income_before_tax = total_non_operating_income - finance_cost
            
            self.logger.info(f"[PN-IS-V9.0] Net Financial Income Before Tax: {net_financial_income_before_tax.to_dict()}")
            
            # ========== 5. VERIFY PRE-TAX RECONCILIATION ==========
            # Operating Income + Financial Income + Exceptional = Profit Before Tax
            profit_before_tax = self._get_safe_series(df, 'Profit Before Tax')
            
            calculated_pbt = operating_income_before_tax + net_financial_income_before_tax + exceptional_items
            
            self.logger.info("\n[PN-IS-V9.0] Pre-tax reconciliation:")
            for year in df.columns:
                reported = profit_before_tax.get(year)
                calculated = calculated_pbt.get(year)
                if pd.notna(reported) and pd.notna(calculated):
                    diff = abs(reported - calculated)
                    self.logger.info(f"  {year}: Reported PBT={reported:.2f}, Calculated={calculated:.2f}, Diff={diff:.2f}")
                    if diff > 1:
                        self.logger.warning(f"  {year}: Pre-tax reconciliation mismatch!")
            
            # ========== 6. TAX ALLOCATION ==========
            self.logger.info("\n[PN-IS-V9.0] Starting tax allocation...")
            
            # Get tax expense
            tax_expense = self._get_safe_series(df, 'Tax Expenses')
            ...
            for year in df.columns:
                pbt = profit_before_tax.get(year)
                tax = tax_expense.get(year)
                
                if pd.notna(pbt) and pd.notna(tax) and pbt != 0:
                    effective_tax_rate[year] = tax / pbt
                else:
                    # For losses, check if there's tax benefit
                    if pd.notna(pbt) and pbt < 0 and pd.notna(tax) and tax < 0:
                        effective_tax_rate[year] = tax / pbt
                    else:
                        effective_tax_rate[year] = 0  # No tax on losses unless specified
            ...
            for year in df.columns:
                eff_rate = effective_tax_rate.get(year, 0)
                
                # Apply effective rate to each component
                tax_on_operating[year] = operating_income_before_tax.get(year, 0) * eff_rate
                tax_on_financial[year] = net_financial_income_before_tax.get(year, 0) * eff_rate
                tax_on_exceptional[year] = exceptional_items.get(year, 0) * eff_rate
                
                # Ensure total allocated equals actual tax
                total_allocated = tax_on_operating[year] + tax_on_financial[year] + tax_on_exceptional[year]
                actual_tax = tax_expense.get(year, 0)
                
                if pd.notna(actual_tax) and pd.notna(total_allocated) and abs(total_allocated) > 0.01:
                    # Adjust for rounding differences
                    adjustment = actual_tax - total_allocated
                    
                    # Apply adjustment to operating tax (largest component usually)
                    tax_on_operating[year] += adjustment
                    
                    self.logger.debug(f"[PN-IS-V9.0] {year}: Tax adjustment of {adjustment:.2f} applied to operating tax")
            ...
            reformulated['Operating Income After Tax'] = operating_income_before_tax - tax_on_operating
            ...
            # ========== 7. STORE DETAILED COMPONENTS ==========
            reformulated['Interest Expense'] = finance_cost
            reformulated['Interest Income'] = pd.Series(0, index=df.columns)  # Not separately available
            reformulated['Other Non-Operating Income'] = total_non_operating_income
            ...
            reformulated['EBIT'] = ebit_with_other_income
            reformulated['Prior Year Adjustments'] = prior_year_adj
            
            # ========== 8. NET INCOME RECONCILIATION ==========
            # Get reported net income
            net_income_reported = self._get_safe_series(df, 'Profit Attributable to Shareholders')
            if net_income_reported is None:
                net_income_reported = self._get_safe_series(df, 'Profit After Tax')
            ...
            self.logger.info("\n[PN-IS-V9.0] Net Income Reconciliation:")
            ...
            for year in df.columns:
                ...
                if pd.notna(reported) and pd.notna(calculated):
                    ...
                    pct_diff = (diff / abs(reported) * 100) if reported != 0 else 0
                    
                    self.logger.info(f"\n[PN-IS-V9.0] {year}:")
                    self.logger.info(f"  Components:")
                    self.logger.info(f"    Operating Income BT: {operating_income_before_tax.get(year):,.2f}")
                    self.logger.info(f"    - Tax on Operating: {tax_on_operating.get(year):,.2f}")
                    self.logger.info(f"    = Operating After Tax: {reformulated['Operating Income After Tax'][year]:,.2f}")
                    self.logger.info(f"    ")
                    self.logger.info(f"    Financial Income BT: {net_financial_income_before_tax.get(year):,.2f}")
                    self.logger.info(f"    - Tax on Financial: {tax_on_financial.get(year):,.2f}")
                    self.logger.info(f"    = Financial After Tax: {reformulated['Net Financial Income After Tax'][year]:,.2f}")
                    self.logger.info(f"    ")
                    self.logger.info(f"    Exceptional BT: {exceptional_items.get(year):,.2f}")
                    self.logger.info(f"    - Tax on Exceptional: {tax_on_exceptional.get(year):,.2f}")
                    self.logger.info(f"    = Exceptional After Tax: {reformulated['Exceptional Items After Tax'][year]:,.2f}")
                    self.logger.info(f"    ")
                    self.logger.info(f"  Total Calculated: {calculated:,.2f}")
                    self.logger.info(f"  Reported Net Income: {reported:,.2f}")
                    self.logger.info(f"  Difference: {diff:,.2f} ({pct_diff:.2f}%)")
            ...
            if reconciliation_errors:
                metadata['reconciliation_errors'] = reconciliation_errors
                self.logger.warning(f"[PN-IS-V9.0] Reconciliation errors found in {len(reconciliation_errors)} years")
            else:
                self.logger.info("[PN-IS-V9.0] Net Income reconciliation successful!")
            
            # ========== 9. ADDITIONAL METRICS ==========
            
            # Gross Profit
            try:
                cost_of_material = self._get_safe_series(df, 'Cost of Material Consumed', default_zero=True)
                inventory_changes = self._get_safe_series(df, 'Changes in Inventories of Finished Goods, Work-in-Progress and Stock-in-Trade', default_zero=True)
                purchases = self._get_safe_series(df, 'Purchases of Stock-in-Trade', default_zero=True)
                
                # Calculate COGS (note: inventory increase is negative in P&L)
                cogs = cost_of_material + inventory_changes + purchases
                gross_profit = revenue_net - cogs
                
                reformulated['Gross Profit'] = gross_profit
                
                # Gross margin
                gross_margin = (gross_profit / revenue_net * 100).round(2)
                self.logger.info(f"[PN-IS-V9.0] Gross Margin %: {gross_margin.to_dict()}")
                
            except Exception as e:
                self.logger.debug(f"[PN-IS-V9.0] Could not calculate gross profit: {e}")
            
            # EBITDA
            try:
                depreciation = self._get_safe_series(df, 'Depreciation and Amortization', default_zero=True)
                if (depreciation != 0).any():
                    # EBITDA = Operating Income Before Tax + D&A
                    ebitda = operating_income_before_tax + depreciation
                    reformulated['EBITDA'] = ebitda
                    
                    # EBITDA margin
                    ebitda_margin = (ebitda / revenue_net * 100).round(2)
                    self.logger.info(f"[PN-IS-V9.0] EBITDA Margin %: {ebitda_margin.to_dict()}")
                    
            except Exception as e:
                self.logger.debug(f"[PN-IS-V9.0] Could not calculate EBITDA: {e}")
            ...
            for year in df.columns:
                total_tax_allocated = (
                    tax_on_operating.get(year, 0) + 
                    tax_on_financial.get(year, 0) + 
                    tax_on_exceptional.get(year, 0)
                )
                actual_tax = tax_expense.get(year, 0)
                
                if pd.notna(actual_tax) and pd.notna(total_tax_allocated):
                    tax_diff = abs(actual_tax - total_tax_allocated)
                    if tax_diff > 0.01:
                        self.logger.error(
                            f"[PN-IS-V9.0] {year}: Tax allocation error! "
                            f"Allocated={total_tax_allocated:.2f}, Actual={actual_tax:.2f}"
                        )
            
            # ========== 11. SUMMARY ==========
            self.logger.info("\n[PN-IS-V9.0-SUMMARY] Income Statement Reformulation Summary:")
            self.logger.info("="*60)
            
            latest_year = df.columns[-1]
            if latest_year in reformulated.columns:
                self.logger.info(f"Latest Year ({latest_year}) Analysis:")
                self.logger.info(f"  Revenue: {revenue_net.get(latest_year):,.0f}")
                self.logger.info(f"  EBIT: {ebit_with_other_income.get(latest_year):,.0f}")
                self.logger.info(f"  Operating Income (before tax): {operating_income_before_tax.get(latest_year):,.0f}")
                self.logger.info(f"  Operating Income (after tax): {reformulated['Operating Income After Tax'].get(latest_year):,.0f}")
                self.logger.info(f"  Net Financial Expense (after tax): {-reformulated['Net Financial Income After Tax'].get(latest_year):,.0f}")
                self.logger.info(f"  Net Income: {net_income_reported.get(latest_year):,.0f}")
                self.logger.info(f"  Effective Tax Rate: {effective_tax_rate.get(latest_year):.1%}")
            
            self.logger.info("\n[PN-IS-V9.0] Key Assumptions:")
            self.logger.info(f"  - Other Income Treatment: {metadata.get('other_income_treatment', 'N/A')}")
            self.logger.info(f"  - Prior Year Adjustments: Allocated to operating income")
            self.logger.info(f"  - Tax Allocation: Based on effective tax rate with reconciliation")
            
        except Exception as e:
            self.logger.error(f"[PN-IS-V9.0-ERROR] Income statement reformulation failed: {e}", exc_info=True)
            raise
        
        finally:
            # Store metadata
            metadata['version'] = '9.0'
            metadata['tax_allocation_method'] = 'Effective rate with reconciliation'
            metadata['key_assumption'] = 'Finance cost added back to get EBIT'
            self.calculation_metadata['income_statement'] = metadata
            
            self.logger.info("\n[PN-IS-V9.0-END] Income Statement Reformulation Complete")
            self.logger.info("="*80 + "\n")
    ...
    def _analyze_other_income_composition(self, other_income_source: str, df: pd.DataFrame) -> bool:
        ...
        In Indian accounting context, Other Income typically includes:
        - Interest income (non-operating)
        - Dividend income (non-operating)
        - Profit on sale of investments (non-operating)
        - Foreign exchange gains/losses (can be either)
        - Miscellaneous receipts (can be either)
        """
        if not other_income_source:
            return False
        
        # Get Other Income values
        other_income = self._get_safe_series(df, other_income_source, default_zero=True)
        revenue = self._get_safe_series(df, 'Revenue From Operations(Net)')
        
        if other_income is None or revenue is None:
            return False
        ...
        # Decision logic:
        # 1. If Other Income is < 2% of revenue, likely non-operating (interest/dividends)
        # 2. If Other Income is > 5% of revenue, likely includes operating items
        # 3. High volatility (>0.5) suggests non-operating (investment gains/losses)
        # 4. Low volatility (<0.3) suggests operating (regular business income)
        
        if other_income_pct < 2:
            return False  # Small amount, likely just interest/dividends
        elif other_income_pct > 5:
            return True   # Large amount, likely includes operating items
        elif other_income_volatility < 0.3:
            return True   # Stable income, likely operating
        else:
            return False  # Default to non-operating for conservative analysis
    ...
    def _calculate_ratios_enhanced(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Version 17.0 - Final robust implementation with comprehensive Interest Coverage handling
        """
        
        self.logger.info("\n" + "="*80)
        self.logger.info("[PN-RATIOS-V17.0] Starting Final Enhanced Ratio Calculations")
        self.logger.info("="*80)
        ...
        self.logger.info(f"[PN-RATIOS-V17.0] Input data shapes - BS: {ref_bs.shape}, IS: {ref_is.shape}")
        ...
        metadata = {
            ...
            'nbc_details': {},
            'financial_position': {},
            'interest_coverage_method': {}
        }
        
        try:
            # ========== 1. CORE OPERATING PERFORMANCE METRICS ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === OPERATING PERFORMANCE METRICS ===")
            ...
            if 'Operating Income After Tax' in ref_is.index and 'Net Operating Assets' in ref_bs.index:
                try:
                    ...
                    # Calculate average NOA
                    avg_noa = pd.Series(index=ref_bs.columns, dtype=float)
                    for i, year in enumerate(ref_bs.columns):
                        if i == 0:
                            avg_noa[year] = noa[year]
                        else:
                            prev_year = ref_bs.columns[i-1]
                            avg_noa[year] = (noa[prev_year] + noa[year]) / 2
                    
                    # Calculate RNOA
                    rnoa = pd.Series(index=ref_bs.columns, dtype=float)
                    
                    for year in ref_bs.columns:
                        if pd.notna(avg_noa[year]) and pd.notna(nopat[year]):
                            if abs(avg_noa[year]) > 10:
                                rnoa[year] = (nopat[year] / avg_noa[year]) * 100
                            else:
                                rnoa[year] = np.nan
                                self.logger.warning(f"[PN-RATIOS-V17.0] {year}: NOA near zero, RNOA undefined")
                    
                    ratios['Return on Net Operating Assets (RNOA) %'] = rnoa
                    rnoa_calculated = True
                    metadata['calculation_status']['successful'].append('RNOA')
                    ...
                    if 'Revenue' in ref_is.index:
                        ...
                        for year in ref_bs.columns:
                            if pd.notna(revenue[year]) and pd.notna(nopat[year]) and revenue[year] > 0:
                                opm[year] = (nopat[year] / revenue[year]) * 100
                        ...
                        for year in ref_bs.columns:
                            if pd.notna(avg_noa[year]) and pd.notna(revenue[year]) and abs(avg_noa[year]) > 10:
                                noat[year] = revenue[year] / avg_noa[year]
                        
                        ratios['Net Operating Asset Turnover (NOAT)'] = noat
                        
                        # Verify decomposition
                        for year in ref_bs.columns:
                            if all(pd.notna(x[year]) for x in [rnoa, opm, noat]):
                                calculated_rnoa = (opm[year] * noat[year]) / 100
                                diff = abs(rnoa[year] - calculated_rnoa)
                                if diff > 0.1:
                                    self.logger.warning(f"[PN-RATIOS-V17.0] {year}: RNOA decomposition mismatch: {diff:.2f}%")
                    
                except Exception as e:
                    self.logger.error(f"[PN-RATIOS-V17.0] RNOA calculation failed: {e}")
            ...
            self.logger.info("\n[PN-RATIOS-V17.0] === FINANCIAL LEVERAGE METRICS ===")
            ...
            if 'Net Financial Assets' in ref_bs.index and 'Common Equity' in ref_bs.index:
                try:
                    ...
                    flev_status = pd.Series(index=ref_bs.columns, dtype=str)
                    
                    for year in ref_bs.columns:
                        if pd.notna(avg_ce[year]) and pd.notna(nfa[year]) and abs(avg_ce[year]) > 10:
                            flev[year] = -nfa[year] / avg_ce[year]
                    ...
                    metadata['leverage_status'] = flev_status.to_dict()
                    metadata['calculation_status']['successful'].append('FLEV')
                    
                except Exception as e:
                    self.logger.error(f"[PN-RATIOS-V17.0] FLEV calculation failed: {e}")
            ...
            # ========== 3. COMPREHENSIVE FINANCIAL INCOME/EXPENSE CALCULATION ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === COMPREHENSIVE FINANCIAL ANALYSIS ===")
            
            # Get all available financial data sources
            financial_expense = pd.Series(0.0, index=ref_bs.columns)
            financial_income = pd.Series(0.0, index=ref_bs.columns)
            
            # 1. Financial Expense Sources (in order of preference)
            expense_sources = [
                'Interest Expense',
                'Finance Cost', 
                'Interest Paid',
            ...
            financial_expense_found = False
            for source in expense_sources:
                if source in ref_is.index:
                    financial_expense = ref_is.loc[source].fillna(0)
                    financial_expense_found = True
                    self.logger.info(f"[PN-RATIOS-V17.0] Using {source} for financial expense")
                    break
                else:
                    # Try original data
                    try:
                        expense_data = self._get_safe_series(self._df_clean, source, default_zero=True)
                        if expense_data is not None and expense_data.sum() > 0:
                            financial_expense = expense_data
                            financial_expense_found = True
                            self.logger.info(f"[PN-RATIOS-V17.0] Using {source} from original data for financial expense")
                            break
                    except:
                        continue
            
            if not financial_expense_found:
                self.logger.info("[PN-RATIOS-V17.0] No financial expense found - company appears debt-free")
            
            # 2. Financial Income Estimation
            try:
                other_income = self._get_safe_series(self._df_clean, 'Other Income', default_zero=True)
                self.logger.info(f"[PN-RATIOS-V17.0] Other Income: {other_income.to_dict()}")
            except:
                other_income = pd.Series(0.0, index=ref_bs.columns)
            
            # Get financial assets components
            current_investments = ref_bs.loc['Current Investments'] if 'Current Investments' in ref_bs.index else pd.Series(0.0, index=ref_bs.columns)
            cash_and_equiv = ref_bs.loc['Cash and Equivalents'] if 'Cash and Equivalents' in ref_bs.index else pd.Series(0.0, index=ref_bs.columns)
            lt_investments = ref_bs.loc['Long-term Investments'] if 'Long-term Investments' in ref_bs.index else pd.Series(0.0, index=ref_bs.columns)
            bank_balances = ref_bs.loc['Bank Balances'] if 'Bank Balances' in ref_bs.index else pd.Series(0.0, index=ref_bs.columns)
            
            # Total financial assets
            total_financial_assets = current_investments + cash_and_equiv + lt_investments + bank_balances
            
            # Enhanced financial income estimation
            for year in ref_bs.columns:
                if other_income[year] > 0:
                    if total_financial_assets[year] > 0:
                        implied_rate = other_income[year] / total_financial_assets[year]
                        # Conservative approach: if rate is reasonable, use most of Other Income
                        if implied_rate <= 0.12:  # Up to 12% return is reasonable
                            financial_income[year] = other_income[year] * 0.85  # 85% of Other Income
                        else:  # If rate seems too high, use market rate
                            financial_income[year] = total_financial_assets[year] * 0.065  # 6.5% market return
                    else:
                        financial_income[year] = other_income[year] * 0.6  # Conservative default
                
                self.logger.info(f"[PN-RATIOS-V17.0] {year}: Other Income={other_income[year]:.0f}, "
                               f"Financial Assets={total_financial_assets[year]:.0f}, "
                               f"Estimated Financial Income={financial_income[year]:.0f}, "
                               f"Financial Expense={financial_expense[year]:.0f}")
            
            # 3. Tax Rate Calculation
            tax_rate = pd.Series(0.25, index=ref_bs.columns)  # Default 25%
            
            try:
                pbt = self._get_safe_series(self._df_clean, 'Profit Before Tax')
                tax_expense = self._get_safe_series(self._df_clean, 'Tax Expenses')
                
                if pbt is not None and tax_expense is not None:
                    for year in ref_bs.columns:
                        if pd.notna(pbt[year]) and pd.notna(tax_expense[year]) and pbt[year] > 0:
                            calculated_rate = tax_expense[year] / pbt[year]
                            if 0.10 <= calculated_rate <= 0.45:  # Reasonable tax rate range
                                tax_rate[year] = calculated_rate
                            else:
                                tax_rate[year] = 0.25  # Default fallback
                        else:
                            tax_rate[year] = 0.25
                else:
                    # Try from reformulated data
                    if 'Effective Tax Rate (Info Only)' in ref_is.index:
                        ref_tax_rate = ref_is.loc['Effective Tax Rate (Info Only)']
                        for year in ref_bs.columns:
                            if pd.notna(ref_tax_rate[year]) and 0.10 <= ref_tax_rate[year] <= 0.45:
                                tax_rate[year] = ref_tax_rate[year]
            except Exception as e:
                self.logger.warning(f"[PN-RATIOS-V17.0] Tax rate calculation failed: {e}, using default 25%")
            ...
            self.logger.info("\n[PN-RATIOS-V17.0] === NET BORROWING COST ===")
            ...
            try:
                if 'Net Financial Assets' in ref_bs.index and 'Total Assets' in ref_bs.index:
                    ...
                    # Calculate average NFA
                    avg_nfa = pd.Series(index=ref_bs.columns, dtype=float)
                    for i, year in enumerate(ref_bs.columns):
                        if i == 0:
                            avg_nfa[year] = nfa[year]
                        else:
                            prev_year = ref_bs.columns[i-1]
                            avg_nfa[year] = (nfa[prev_year] + nfa[year]) / 2
                    
                    avg_nfo = -avg_nfa  # Net Financial Obligations
                    ...
                    for year in ref_bs.columns:
                        ...
                        if pd.notna(nfo) and pd.notna(nfe) and pd.notna(ta) and pd.notna(nfa_val):
                            threshold = 0.001 * ta
                            ...
                            if abs(nfa_val) > threshold and abs(nfo) > threshold:
                                nbc[year] = (nfe / nfo) * 100
                                
                                if nfa_val > 0:  # Cash-rich
                                    if nfe < 0:
                                ...
                                # Apply bounds
                                nbc[year] = np.clip(nbc[year], -15, 25)
                            else:
                                nbc[year] = 0.0
                    ...
                    nbc_calculated = True
                    metadata['calculation_status']['successful'].append('NBC')
                    metadata['nbc_interpretation'] = nbc_interpretation.to_dict()
                    
            except Exception as e:
                self.logger.error(f"[NBC] Calculation failed: {e}", exc_info=True)
                metadata['calculation_status']['failed'].append(f'NBC: {str(e)}')
            
            ratios['Net Borrowing Cost (NBC) %'] = nbc
            
            # ========== 5. ADDITIONAL FINANCIAL METRICS ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === ADDITIONAL FINANCIAL METRICS ===")
            ...
            try:
                ...
                self.logger.error(f"[PN-RATIOS-V17.0] Gross rate calculation failed: {e}")
            ...
            try:
                if total_financial_assets.sum() > 0 and financial_income.sum() > 0:
                    avg_fin_assets = pd.Series(index=ref_bs.columns, dtype=float)
                    for i, year in enumerate(ref_bs.columns):
                        if i == 0:
                            avg_fin_assets[year] = total_financial_assets[year]
                        else:
                            prev_year = ref_bs.columns[i-1]
                            avg_fin_assets[year] = (total_financial_assets[prev_year] + total_financial_assets[year]) / 2
                    ...
                    for year in ref_bs.columns:
                        if pd.notna(avg_fin_assets[year]) and pd.notna(financial_income[year]) and avg_fin_assets[year] > 10:
                            ...
                            fin_asset_return[year] = np.clip(fin_asset_return[year], 0, 12)
                    
                    metadata['calculation_status']['successful'].append('Return on Financial Assets')
                    
            except Exception as e:
                self.logger.warning(f"[PN-RATIOS-V17.0] Financial asset return calculation failed: {e}")
            
            ratios['Return on Financial Assets %'] = fin_asset_return
            
            # ========== 6. SPREAD CALCULATION ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === SPREAD CALCULATION ===")
            
            spread = pd.Series(0.0, index=ref_bs.columns)
            spread_interpretation = pd.Series(index=ref_bs.columns, dtype=str)
            
            if rnoa_calculated and nbc_calculated:
                try:
                    ...
                    self.logger.error(f"[PN-RATIOS-V17.0] Spread calculation failed: {e}")
            
            ratios['Spread %'] = spread
            ratios['Leverage Spread %'] = spread
            
            # ========== 7. ROE AND DECOMPOSITION ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === ROE AND DECOMPOSITION ===")
            ...
            try:
                if 'Net Income (Reported)' in ref_is.index and 'Common Equity' in ref_bs.index:
                    ...
                    equity = ref_bs.loc['Common Equity']
                    ...
                    roe_calculated = True
                    metadata['calculation_status']['successful'].append('ROE')
                    ...
                    if rnoa_calculated and flev_calculated:
                        ...
                        for year in ref_bs.columns:
                            if all(pd.notna(x[year]) for x in [roe, calculated_roe]):
                                diff = abs(roe[year] - calculated_roe[year])
                                if diff > 1.0:
                                    decomposition_errors.append({
                                        'year': year,
                                        'reported_roe': roe[year],
                                        'calculated_roe': calculated_roe[year],
                                        'difference': diff
                                    })
                        
                        if decomposition_errors:
                            metadata['roe_decomposition_errors'] = decomposition_errors
                    else:
                        ratios['ROE (Calculated) %'] = pd.Series(np.nan, index=ref_bs.columns)
                        
            except Exception as e:
                self.logger.error(f"[PN-RATIOS-V17.0] ROE calculation failed: {e}")
            ...
            self.logger.info("\n[PN-RATIOS-V17.0] === RETURN ON ASSETS ===")
            ...
            try:
                if 'Net Income (Reported)' in ref_is.index and 'Total Assets' in ref_bs.index:
                    ...
                    for i, year in enumerate(ref_bs.columns):
                        if i == 0:
                            avg_assets[year] = total_assets[year]
                        else:
                            prev_year = ref_bs.columns[i-1]
                            avg_assets[year] = (total_assets[prev_year] + total_assets[year]) / 2
                    
                    roa = pd.Series(index=ref_bs.columns, dtype=float)
                    for year in ref_bs.columns:
                        if pd.notna(avg_assets[year]) and pd.notna(net_income[year]) and avg_assets[year] > 0:
                            roa[year] = (net_income[year] / avg_assets[year]) * 100
                    
                    ratios['Return on Assets (ROA) %'] = roa
                    metadata['calculation_status']['successful'].append('ROA')
                    
            except Exception as e:
                self.logger.error(f"[PN-RATIOS-V17.0] ROA calculation failed: {e}")
            ...
            # ========== 10. GROWTH METRICS ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === GROWTH METRICS ===")
            ...
            try:
                if 'Net Income (Reported)' in ref_is.index:
                    ...
                    metadata['calculation_status']['successful'].append('Net Income Growth')
                else:
                    ratios['Net Income Growth %'] = pd.Series(np.nan, index=ref_bs.columns)
            except Exception as e:
                self.logger.error(f"[PN-RATIOS-V17.0] Net Income Growth calculation failed: {e}")
                ratios['Net Income Growth %'] = pd.Series(np.nan, index=ref_bs.columns)
            ...
            if 'Revenue' in ref_is.index and 'Total Assets' in ref_bs.index:
                ...
                for year in ref_bs.columns:
                    if pd.notna(avg_assets[year]) and pd.notna(revenue[year]) and avg_assets[year] > 0:
                        asset_turnover[year] = revenue[year] / avg_assets[year]
                
                ratios['Asset Turnover'] = asset_turnover
            
            # Working Capital Turnover
            working_capital_turnover = pd.Series(np.nan, index=ref_bs.columns)
            try:
                current_assets = self._get_safe_series(self._df_clean, 'Current Assets')
                current_liabilities = self._get_safe_series(self._df_clean, 'Current Liabilities')
                
                if current_assets is not None and current_liabilities is not None and 'Revenue' in ref_is.index:
                    revenue = ref_is.loc['Revenue']
                    working_capital = current_assets - current_liabilities
                    
                    avg_wc = pd.Series(index=ref_bs.columns, dtype=float)
                    for i, year in enumerate(ref_bs.columns):
                        if i == 0:
                            avg_wc[year] = working_capital[year]
                        else:
                            prev_year = ref_bs.columns[i-1]
                            avg_wc[year] = (working_capital[prev_year] + working_capital[year]) / 2
                    
                    for year in ref_bs.columns:
                        if pd.notna(avg_wc[year]) and pd.notna(revenue[year]) and avg_wc[year] > 0:
                            working_capital_turnover[year] = revenue[year] / avg_wc[year]
            except:
                pass
                
            ratios['Working Capital Turnover'] = working_capital_turnover
            ...
            try:
                ...
                if current_assets is not None and current_liabilities is not None:
                    ...
                    for year in ref_bs.columns:
                        if pd.notna(current_liabilities[year]) and current_liabilities[year] > 0:
                            current_ratio[year] = current_assets[year] / current_liabilities[year]
                    
                    ratios['Current Ratio'] = current_ratio
                    
                    # Quick Ratio
                    inventory = self._get_safe_series(self._df_clean, 'Inventories', default_zero=True)
                    if inventory is not None:
                        ...
                        for year in ref_bs.columns:
                            if pd.notna(current_liabilities[year]) and current_liabilities[year] > 0:
                                quick_ratio[year] = quick_assets[year] / current_liabilities[year]
                    ...
                    if 'Cash and Equivalents' in ref_bs.index:
                        ...
                        for year in ref_bs.columns:
                            if pd.notna(current_liabilities[year]) and current_liabilities[year] > 0:
                                cash_ratio[year] = cash[year] / current_liabilities[year]
                        
                        ratios['Cash Ratio'] = cash_ratio
                    else:
                        ratios['Cash Ratio'] = pd.Series(np.nan, index=ref_bs.columns)
                else:
                    ratios['Current Ratio'] = pd.Series(np.nan, index=ref_bs.columns)
                    ratios['Quick Ratio'] = pd.Series(np.nan, index=ref_bs.columns)
                    ratios['Cash Ratio'] = pd.Series(np.nan, index=ref_bs.columns)
            except Exception as e:
                self.logger.error(f"[PN-RATIOS-V17.0] Liquidity ratios calculation failed: {e}")
                ratios['Current Ratio'] = pd.Series(np.nan, index=ref_bs.columns)
                ratios['Quick Ratio'] = pd.Series(np.nan, index=ref_bs.columns)
                ratios['Cash Ratio'] = pd.Series(np.nan, index=ref_bs.columns)
            
            # ========== 13. COMPREHENSIVE INTEREST COVERAGE - ROBUST SOLUTION ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === COMPREHENSIVE INTEREST COVERAGE ANALYSIS ===")
            
            interest_coverage = pd.Series(np.nan, index=ref_bs.columns)
            
            try:
                # Get EBIT/Operating Income Before Tax
                ebit = None
                ebit_sources = ['Operating Income Before Tax', 'EBIT', 'Earnings Before Interest and Tax']
                
                for source in ebit_sources:
                    if source in ref_is.index:
                        ebit = ref_is.loc[source]
                        self.logger.info(f"[PN-RATIOS-V17.0] Using {source} for EBIT: {ebit.to_dict()}")
                        break
                
                if ebit is None:
                    # Try to calculate EBIT from original data
                    try:
                        pbt = self._get_safe_series(self._df_clean, 'Profit Before Tax')
                        finance_cost = self._get_safe_series(self._df_clean, 'Finance Cost', default_zero=True)
                        if pbt is not None:
                            ebit = pbt + finance_cost  # EBIT = PBT + Interest Expense
                            self.logger.info(f"[PN-RATIOS-V17.0] Calculated EBIT from PBT + Finance Cost: {ebit.to_dict()}")
                    except:
                        pass
                
                if ebit is not None:
                    # Comprehensive Interest Coverage Logic
                    for year in ref_bs.columns:
                        ebit_val = ebit[year]
                        fin_exp = financial_expense[year]
                        
                        if pd.notna(ebit_val) and pd.notna(fin_exp):
                            # Check debt position
                            debt_position = metadata.get('financial_position', {}).get(year, 'unknown')
                            
                            if fin_exp > 0.01:  # Company has meaningful interest expense
                                # Standard calculation
                                coverage = ebit_val / fin_exp
                                # Cap at reasonable levels for display
                                interest_coverage[year] = min(max(coverage, -999), 999)
                                metadata['interest_coverage_method'][year] = 'Standard: EBIT/Interest Expense'
                                
                                self.logger.info(f"[PN-RATIOS-V17.0] {year}: Standard Interest Coverage = {ebit_val:.0f}/{fin_exp:.2f} = {interest_coverage[year]:.1f}")
                                
                            elif debt_position == 'cash_rich' or fin_exp == 0:
                                # Debt-free company - Interest Coverage is conceptually infinite
                                # But we need to check if there's any debt at all
                                
                                total_debt_val = 0
                                if 'Total Debt' in ref_bs.index:
                                    total_debt_val = ref_bs.loc['Total Debt', year]
                                
                                if total_debt_val <= 10:  # Essentially debt-free (allowing for rounding)
                                    if ebit_val > 0:
                                        interest_coverage[year] = 999  # Very high number indicating strong coverage
                                        metadata['interest_coverage_method'][year] = 'Debt-free: Set to 999 (excellent)'
                                        self.logger.info(f"[PN-RATIOS-V17.0] {year}: Debt-free company with positive EBIT = 999 (excellent coverage)")
                                    else:
                                        interest_coverage[year] = np.nan  # No earnings, so coverage undefined
                                        metadata['interest_coverage_method'][year] = 'Debt-free but negative EBIT: N/A'
                                        self.logger.info(f"[PN-RATIOS-V17.0] {year}: Debt-free but negative EBIT = N/A")
                                else:
                                    # Has debt but no interest expense (unusual)
                                    if ebit_val > 0:
                                        interest_coverage[year] = 99  # High but not infinite due to potential future interest
                                        metadata['interest_coverage_method'][year] = 'Has debt but no interest expense'
                                        self.logger.warning(f"[PN-RATIOS-V17.0] {year}: Has debt ({total_debt_val:.0f}) but no interest expense")
                                    else:
                                        interest_coverage[year] = np.nan
                                        metadata['interest_coverage_method'][year] = 'Negative EBIT with debt: N/A'
                            else:
                                # Edge case: has some financial expense but very small
                                if ebit_val > 0:
                                    coverage = ebit_val / max(fin_exp, 0.01)  # Avoid division by zero
                                    interest_coverage[year] = min(coverage, 999)
                                    metadata['interest_coverage_method'][year] = 'Small interest expense: Calculated with floor'
                                    self.logger.info(f"[PN-RATIOS-V17.0] {year}: Small interest expense calculation = {interest_coverage[year]:.1f}")
                                else:
                                    interest_coverage[year] = np.nan
                                    metadata['interest_coverage_method'][year] = 'Negative EBIT: N/A'
                        else:
                            interest_coverage[year] = np.nan
                            metadata['interest_coverage_method'][year] = 'Missing EBIT or financial expense data'
                            self.logger.warning(f"[PN-RATIOS-V17.0] {year}: Missing data - EBIT={ebit_val}, Fin Exp={fin_exp}")
                    
                    metadata['calculation_status']['successful'].append('Interest Coverage')
                    self.logger.info(f"[PN-RATIOS-V17.0] Final Interest Coverage: {interest_coverage.to_dict()}")
                    
                else:
                    # Cannot calculate without EBIT
                    self.logger.error("[PN-RATIOS-V17.0] Cannot find EBIT data for Interest Coverage calculation")
                    metadata['calculation_status']['failed'].append('Interest Coverage: No EBIT data')
                    for year in ref_bs.columns:
                        metadata['interest_coverage_method'][year] = 'Failed: No EBIT data'
                
            except Exception as e:
                self.logger.error(f"[PN-RATIOS-V17.0] Interest Coverage calculation failed: {e}", exc_info=True)
                metadata['calculation_status']['failed'].append(f'Interest Coverage: {str(e)}')
                for year in ref_bs.columns:
                    metadata['interest_coverage_method'][year] = f'Failed: {str(e)}'
            ...
            try:
                if 'Revenue' in ref_is.index:
                    ...
                    if 'Net Income (Reported)' in ref_is.index:
                        ...
                        for year in ref_bs.columns:
                            if pd.notna(revenue[year]) and revenue[year] > 0:
                                npm[year] = (net_income[year] / revenue[year]) * 100
                        ratios['Net Profit Margin %'] = npm
                    else:
                        ratios['Net Profit Margin %'] = pd.Series(np.nan, index=ref_bs.columns)
            except Exception as e:
                self.logger.error(f"[PN-RATIOS-V17.0] Profitability margins calculation failed: {e}")
                ratios['Gross Profit Margin %'] = pd.Series(np.nan, index=ref_bs.columns)
                ratios['EBITDA Margin %'] = pd.Series(np.nan, index=ref_bs.columns)
                ratios['Net Profit Margin %'] = pd.Series(np.nan, index=ref_bs.columns)
            ...
            # Ensure all required ratios exist
            required_ratios = [
            ...
            for ratio_name in required_ratios:
                if ratio_name not in ratios.index:
                    self.logger.info(f"[PN-RATIOS-V17.0] Adding missing ratio: {ratio_name}")
                    ratios.loc[ratio_name] = pd.Series(np.nan, index=ratios.columns)
            
            # ========== 16. COMPREHENSIVE VALIDATION AND SUMMARY ==========
            self.logger.info("\n[PN-RATIOS-V17.0] === COMPREHENSIVE VALIDATION ===")
            
            # Check data quality
            total_ratios = len(ratios.index)
            available_ratios = sum(1 for ratio in ratios.index if not ratios.loc[ratio].isna().all())
            coverage_pct = (available_ratios / total_ratios) * 100
            
            self.logger.info(f"[PN-RATIOS-V17.0] Data Coverage: {available_ratios}/{total_ratios} ({coverage_pct:.1f}%)")
            
            # Analyze missing ratios
            missing_ratios = [ratio for ratio in ratios.index if ratios.loc[ratio].isna().all()]
            if missing_ratios:
                self.logger.warning(f"[PN-RATIOS-V17.0] Missing ratios: {missing_ratios}")
            
            # Key ratio validation for VST (cash-rich company)
            latest_year = ratios.columns[-1] if len(ratios.columns) > 0 else None
            if latest_year:
                self.logger.info(f"\n[PN-RATIOS-V17.0] Key Ratios Validation ({latest_year}):")
                
                key_validations = {
                    'Return on Net Operating Assets (RNOA) %': (10, 100),  # Should be positive for profitable company
                    'Financial Leverage (FLEV)': (-2, 0),  # Should be negative for cash-rich
                    'Net Borrowing Cost (NBC) %': (-10, 5),  # Should be negative (earning on cash)
                    'Return on Equity (ROE) %': (10, 50),  # Should be positive and reasonable
                    'Interest Coverage': (50, 999),  # Should be very high for debt-free company
                    'Current Ratio': (1, 10),  # Should be > 1 for healthy liquidity
                }
                
                for ratio_name, (min_val, max_val) in key_validations.items():
                    if ratio_name in ratios.index:
                        value = ratios.loc[ratio_name, latest_year]
                        if pd.notna(value):
                            if min_val <= value <= max_val:
                                status = "✓ PASS"
                            else:
                                status = "⚠ CHECK"
                            self.logger.info(f"  {ratio_name}: {value:.2f} {status}")
                        else:
                            self.logger.warning(f"  {ratio_name}: N/A ⚠ MISSING")
            
            # Final summary
            self.logger.info("\n[PN-RATIOS-V17.0-FINAL-SUMMARY]:")
            ...
            self.logger.info(f"  Warnings: {len(metadata['calculation_status']['warnings'])}")
            self.logger.info(f"  Data coverage: {coverage_pct:.1f}%")
            
            if metadata['calculation_status']['successful']:
                self.logger.info(f"  Successful: {metadata['calculation_status']['successful']}")
            if metadata['calculation_status']['failed']:
                self.logger.error(f"  Failed: {metadata['calculation_status']['failed']}")
            if metadata['calculation_status']['warnings']:
                self.logger.warning(f"  Warnings: {metadata['calculation_status']['warnings']}")
            
        except Exception as e:
            self.logger.error(f"[PN-RATIOS-V17.0-CRITICAL-ERROR] {e}", exc_info=True)
            # Return skeleton DataFrame
            required_ratios = [
                'Return on Net Operating Assets (RNOA) %', 'Operating Profit Margin (OPM) %',
                'Net Operating Asset Turnover (NOAT)', 'Financial Leverage (FLEV)',
                'Financial Leverage (End of Period)', 'Net Borrowing Cost (NBC) %',
                'Spread %', 'Leverage Spread %', 'Return on Equity (ROE) %',
                'ROE (Calculated) %', 'Return on Assets (ROA) %', 'Gross Borrowing Rate %',
                'Return on Financial Assets %', 'Debt to Equity', 'Net Debt to Equity',
                'Revenue Growth %', 'NOA Growth %', 'Net Income Growth %', 'Asset Turnover',
                'Working Capital Turnover', 'Current Ratio', 'Quick Ratio', 'Cash Ratio',
                'Interest Coverage', 'Gross Profit Margin %', 'EBITDA Margin %', 'Net Profit Margin %'
            ]
            ...
            self.logger.info("\n[PN-RATIOS-V17.0-END] Final Enhanced Ratio Calculations Complete")
            self.logger.info("="*80 + "\n")
    ...
    def _analyze_ratio_trends(self, ratios: pd.DataFrame) -> Dict[str, Any]:
        """
        Analyze trends in financial ratios using linear regression
        """
        trends = {}
                ...
                if len(series) >= 3:
                    ...
                    if not np.isnan(y).all():
                        try:
                            ...
                            if series.iloc[0] != 0:
                                cagr = ((series.iloc[-1] / series.iloc[0]) ** (1/len(series)) - 1) * 100
                            else:
                                cagr = np.nan
                            ...
                            trends[ratio_name] = {'trend': 'undefined'}
                    else:
                        trends[ratio_name] = {'trend': 'insufficient_data'}
        
        return trends
    
    def _calculate_ratio_quality_scores(self, ratios: pd.DataFrame) -> Dict[str, float]:
        """
        Calculate quality scores for ratio calculations
        """
        quality_scores = {}
        
        reasonable_ranges = {
            'Return on Net Operating Assets (RNOA) %': (-50, 100),
            'Financial Leverage (FLEV)': (-2, 5),
            'Net Borrowing Cost (NBC) %': (-20, 30),
            'Spread %': (-50, 50),
            'Return on Equity (ROE) %': (-100, 100),
            'Gross Profit Margin %': (0, 80),
            'Net Profit Margin %': (-50, 50),
            'Current Ratio': (0, 10),
            'Interest Coverage': (-10, 1000),
            'Asset Turnover': (0, 10)
        }
        ...
        for ratio_name in ratios.index:
            ...
            completeness = (~series.isna()).sum() / len(series) if len(series) > 0 else 0
            
            series_clean = series.dropna()
            if len(series_clean) > 0 and series_clean.std() > 0 and series_clean.mean() != 0:
                cv = series_clean.std() / abs(series_clean.mean())
                stability = 1 / (1 + cv)
            else:
                stability = 1 if (~series.isna()).any() else 0
            
            if ratio_name in reasonable_ranges:
                min_val, max_val = reasonable_ranges[ratio_name]
                in_range = ((series_clean >= min_val) & (series_clean <= max_val)).sum()
                reasonableness = in_range / len(series_clean) if len(series_clean) > 0 else 0
            else:
                reasonableness = 1
            
            quality_score = (completeness * 0.4 + stability * 0.3 + reasonableness * 0.3) * 100
            quality_scores[ratio_name] = round(quality_score, 2)
        
        return quality_scores
    
    def _log_calculation(self, metric_name: str, formula: str, 
                        components: Dict[str, pd.Series], 
                        result: pd.Series, 
                        metadata: Dict[str, Any] = None) -> None:
        """
        Log detailed calculation information for debugging and transparency
        """
        ...
        for comp_name, comp_values in components.items():
            if isinstance(comp_values, pd.Series):
                preview = comp_values.head(3).to_dict()
                self.logger.debug(f"  {comp_name}: {preview}")
        
        if isinstance(result, pd.Series):
            preview = result.head(3).to_dict()
